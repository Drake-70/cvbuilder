...
export default function BeforeAfterGaps({ originalText, tailoredCV, coverLetter, gaps = [], language = 'en' }) {
  ...
  const chipClass = (isActive, matched) =>
    `inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium cursor-pointer transition-all border ${
      isActive
        ...
        : 'bg-surface-50 border-surface-200 text-surface-500 hover:border-surface-300 dark:bg-surface-800 dark:border-surface-700 dark:text-surface-400'
    }`;
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid md:grid-cols-2 gap-4 items-start">
        ...
        <div>
          ...
          <div className="rounded-xl border border-surface-200 dark:border-surface-700 bg-surface-0 dark:bg-surface-800 p-4">
            <p className="font-mono text-xs leading-relaxed text-surface-600 dark:text-surface-400 whitespace-pre-wrap max-h-96 overflow-y-auto">
      ...
      {selected !== null && selected !== 'all' && analyzed[selected]?.matched.length === 0 && (
        <div className="mt-4 flex items-start gap-3 p-3 rounded-xl bg-amber-50/80 border border-amber-200 dark:bg-amber-900/20 dark:border-amber-700/40 animate-scale-in">
